                                ...
                                <td class="py-4">
                                    <div class="flex items-center">
                                        <img class="h-16 w-16 mr-4"
                                            src="{{ url('storage', $order_item->product->images[0]) }}"
                                            alt="{{ $order_item->product->name }}">
                                        <span class="font-semibold">{{ $order_item->product->name }}</span>
                                    </div>
                                </td>
                                <td class="py-4">{{ Number::currency($order_item->unit_amount ?? 0, 'IDR', 'id') }}</td>
                                <td class="py-4">
                                    <span class="text-center w-8">{{ $order_item->quantity }}</span>
                                </td>
                                <td class="py-4">{{ Number::currency($order_item->total_amount ?? 0, 'IDR', 'id') }}</td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
        ...
        <div class="md:w-1/4">
            <div class="bg-white rounded-lg shadow-md p-6">
                ...
                <div class="flex justify-between mb-2">
                    <span>Subtotal</span>
                    <span>{{ Number::currency($order_items->sum('total_amount'), 'IDR', 'id') }}</span>
                </div>
                <div class="flex justify-between mb-2">
                    <span>Taxes (1% of Subtotal)</span>
                    <span>{{ Number::currency($order->tax_amount ?? 0.01 * $order_items->sum('total_amount'), 'IDR', 'id') }}</span>
                </div>
                <div class="flex justify-between mb-2">
                    <span>Shipping Costs</span>
                    <span>{{Number::currency($order->shipping_amount ?? config('checkout.shipping_cost'), 'IDR', 'id')}}</span>
                </div>
                <hr class="my-2">
                <div class="flex justify-between mb-2">
                    <span class="font-semibold">Grand Total</span>
                    <span class="font-semibold">{{ Number::currency($order->grand_total, 'IDR', 'id') }}</span>
                </div>
